...
| # | 항목 | 상태 |
|---|---|---|
| 1 | 업데이트 카드 「바뀐 것」이 문단을 쪼개고 파일명을 보여줌 | **고침**(`8cb69d6`) — 릴리즈에 아직 안 실림 |
| 2 | **M1 클린 설치** — 새 macOS 계정에서 온보딩·Gatekeeper·채널·연결 스위치 | **미실시** · 사장님 손 |
| 3 | 자동 업데이트 실제 설치(받기→설치→재시작) | **미실시** — 0.2.0 → 0.2.1이 첫 기회 |
| 4 | M10 콜드 스타트 측정 | 미실시 |
