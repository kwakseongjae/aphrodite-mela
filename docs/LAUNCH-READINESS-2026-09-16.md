...
| 미해명: 첫 실행에 브랜드 킷 모달이 열려 있음 | **닫힘(09-17).** 클린 설치 두 번 모두 안 열렸고, 코드상 클릭 말고는 여는 길이 없다 — 테스트로 고정 |
...
- ~~**브랜드 킷 모달 미해명**~~ — **닫는다(09-17).** "재현 안 됨"으로 닫지 않고 근거를 만들었다: **브랜드 킷을 여는 코드는 호출부가 하나고, 그 한 줄이 액션 스위치의 `case 'brand-kit':` 안에 있다.** 즉 `data-action="brand-kit"`를 누르는 것 말고는 닿을 길이 없고, 부팅 경로에서 그걸 누르는 것은 아무것도 없다. 09-10의 관측은 다른 시트를 잘못 본 것으로 본다.

  테스트로 박았다(`tests/modal-openings.test.ts`) — 호출부가 둘이 되거나 액션 밖으로 나가면 실패한다. 실제로 두 번째 호출부를 넣어 실패하는 것을 확인했다. 스스로 열리는 시트는 첫 실행의 환영 시트 하나뿐이어야 한다는 것도 같이 건다.
