...
**규칙 2 — UI가 완전한 표면이다.** 의미 단위 도구는 UI의 **투영**이지 상위 집합이 아니다. `apply_edits`가 할 수 있는 모든 op는 사람이 누를 수 있는 `data-action`이나 팔레트 항목으로 **존재해야 한다**. (구현까지 DOM 버튼을 경유하면 op마다 undo가 생겨 배치의 원자성이 깨진다. 그래서 실제로는 UI 액션과 **같은 변이 함수**를 쓰고, 대응 관계를 테스트로 고정한다 — `docs/AGENT-TOOLING-BUILD.md` §2.3.) 이 규칙이 깨지면 새 기능이 도구에만 생기고 화면에는 안 생겨서, 라벨을 읽어 조작하는 Astra가 조용히 뒤처진다. 테스트: 모든 op에 대응 액션이 존재하는지 검사.
