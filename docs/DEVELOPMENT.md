...
0. Write `docs/RELEASE-NOTES-<tag>.md` with **What's new before Download** (v0.2.2 confirmed this works: the card opened with three real sentences instead of a DMG filename). The update card summarises the notes, and the summariser that reads them is the one already installed — an older one, without whatever you just fixed. Put the news first and the oldest summariser still opens with news instead of a file name.
1. Bump the version in **four** places: `package.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock` (the `aphrodite-mela` entry), `src-tauri/tauri.conf.json`. The status bar reads it from `package.json` at build time. Miss one and the app and the bundle disagree.
   **`site/script.js` is not one of them.** Its `VERSION` is a *floor*: the last release that is actually published, so the download button works when GitHub's API is rate-limited. `latestRelease()` refuses to go below the floor — that is what stops a stale "latest" downgrading the button — so a floor set to a release still in draft pins the live page to a 404 for the whole notarization window. It happened on v0.2.1, v0.2.2 and v0.2.3. Bump it in step 5, after publishing, and run `node scripts/verify-download.mjs` to check the button resolves.
...
4. Publish, then raise the landing floor to the version you just published and run `node scripts/verify-download.mjs`. Only now does the download button point at the new build; before this it correctly points at the previous one.
   `gh release edit v<version> --draft=false --latest --title "Aphrodite v<version>" --notes-file docs/RELEASE-NOTES-v<version>.md`.
