...
**READY (2026-09-10 13:06 KST).** Draft release `v0.1.0` carries `Aphrodite_0.1.0_aarch64.dmg` (24.3 MB) signed with Developer ID and notarized; the DMG container is notarized and stapled too. Downloaded with a Safari quarantine attribute: `spctl --assess` → `accepted · source=Notarized Developer ID` for both the DMG and the app; `stapler validate` OK for both; the OCR sidecar carries the hardened runtime and a Developer ID signature. Remaining step is publishing the draft release (owner decision) so the landing page's latest-release link resolves.
...
## Home screen feedback (owner, 2026-09-10) — shipped

Sidebar stays fixed and only the project column scrolls (top bar + project header sticky); grid/list toggle persisted per device; card actions moved into a more-menu; outlined SVG star fills gold and pops; duplicate highlights the new card, archive/restore fade the card and toast the project name; sidebar counts for all/favourites/archived with hover states. Native-verified on the rebuilt app; 4 home-view tests.
...
- Tag `v0.1.0` (first attempt, signed path): **failed** — `certificate from APPLE_CERTIFICATE "Apple Development: …" does not match provided identity`. The .p12 exported with `security export -t identities` contained every keychain identity and Tauri reads the first one. `scripts/apple-signing-secrets.sh` now filters through a temporary keychain so the .p12 holds only the Developer ID Application identity; the tag was removed and will be re-pushed after the owner re-runs the script.
- Tag `v0.1.0` (second attempt, Apple ID path): signing succeeded with the filtered Developer ID .p12; notarization first returned **401** (app-specific password), fixed by regenerating it — the helper now pre-checks credentials with `notarytool history` before storing them. The third attempt reached Apple's notary service, which **rejected** the bundle: `Contents/Resources/bin/aphrodite-vision` had no hardened runtime and no secure timestamp because Tauri does not sign `resources`. Fix: the OCR helper is now a Tauri sidecar (`bundle.externalBin`, `src-tauri/bin/aphrodite-vision-<triple>`), placed in `Contents/MacOS/` and signed with the hardened runtime; `src-tauri/src/reference.rs` resolves it next to the app binary (dev: `target/<profile>/`).
- Tag `v0.1.0` (fourth attempt, sidecar fix): **success** — app notarized by tauri-action, but the DMG itself was still `rejected · Unnotarized Developer ID` because tauri-action does not notarize the container. Added `Notarize and staple the DMG` steps (notarytool submit --wait → stapler staple → spctl → `gh release upload --clobber`).
- Tag `v0.1.0` (fifth attempt, current): **success** — https://github.com/kwakseongjae/aphrodite-mela/actions/runs/34435515780. Local verification of the downloaded asset with quarantine: DMG accepted, app accepted, both stapled.
