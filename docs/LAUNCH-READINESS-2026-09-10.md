...
- **Vault snapshots never garbage-collect.** In progress on branch `grok/vault-gc` (Grok 4.6 worker): best-effort prune to `KEEP_SNAPSHOTS = 5` after each sync, current snapshot pinned, orphan dirs removed, three Rust tests. Review + merge pending.
...
- Tag `v0.1.0-rc.2`: re-run pending at the time of writing (draft release, ad-hoc signed until `APPLE_*` secrets are added with `scripts/apple-signing-secrets.sh`).
