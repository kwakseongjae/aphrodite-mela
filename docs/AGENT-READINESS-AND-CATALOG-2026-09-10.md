...
## 7. Owner feedback round (16:00) — home screen

All nine items shipped in one pass and verified natively (`scratchpad/probe/h01–h07`): filters moved from the sidebar to chips beside the search box (전체 / 즐겨찾기 / 보관함 with counts); "Saved to disk / Local files…" replaced by a **저장 위치** button in the top bar with a live saved/unsaved state; the storage modal now shows the path with a copy button, three one-line facts (library.json, library.previous.json, project-vault-v1/) and two backup buttons; EN/한국어 is a dropdown with a check mark; theme/storage/import buttons share a 36px height with lucide icons (sun/moon, hard-drive, file-up), which also fixes the Paper/Midnight height jump; search box is 30px with a 14px lucide icon; focus rings removed globally (inputs show a subtle border colour instead); the "조금씩 완성되어 가는 아이디어들." and "당신만의 관점을 가진 뮤즈." captions are gone.

## 8. v0.1.1 release (2026-09-10 16:15)

CI matrix run 34448418533: both jobs succeeded. Downloaded both assets with a Safari quarantine attribute: `Aphrodite_0.1.1_aarch64.dmg` (24.7 MB) and `Aphrodite_0.1.1_x64.dmg` (24.8 MB) are `accepted · Notarized Developer ID`, stapled, hardened-runtime signed by Developer ID Application (YWQQFQM38J); app and OCR sidecar are arm64 / x86_64 respectively; `LSMinimumSystemVersion 13.0`. Published as the latest release; the landing page's primary button points to the Apple Silicon DMG and shows the Intel link. macOS 26 shows Apple's "Intel-based app support is ending" advisory when launching the x64 build — expected.
