...
- **Floating bottom-right**: the `?` help menu (`details.help-fab > summary`, menu `.help-menu[role=menu]`). Eight entries, in order: **Keyboard shortcuts & commands** (`commands`, ⌘K), **Editor tour** (`tour-start`), **Computer-use guide** (`agent`), **Agent connection** (`agent-connect`, added this month), **Aphrodite Brand Kit** (`brand-kit`), **Release notes** and **Report an issue** (`href` to GitHub, new tab), **Language** (`language-settings`). Esc or a click outside closes it. Home renders the same list without the first four and without Language — see the home section below.
...
## Image scopes, deletion and fonts (added 2026-09-12)

**Scopes.** The local library now has two homes: pictures shared by every project, and pictures filed with one project (`image-library/` and `image-library/projects/<projectId>/`). The Assets tab's My library view gains a scope row (`data-action="local-scope" data-scope="all|project|global"`), two import buttons (`local-import` with `data-scope`), and a delete control per tile (`local-delete` → `local-delete-confirm`). A reference stays `local:<id>`; lookup checks the project's folder first, then the shared one.

**Agent channel.** `POST /agent/library` takes `{action: "list"|"delete"|"import", scope: "project"|"global", id?, name?, base64?}`. List returns every picture with its scope. Import writes bytes into the chosen scope; delete removes the file. Both are receipted (`agent:library-import`, `agent:library-delete`).

**Fonts.** `fonts_installed` lists the families this Mac can render, flagging the ones in the user's own folder. A project may set `system.headingFamily`; `render.ts` turns it into `--heading` through `fontStack`, which always appends a fallback so exported HTML degrades. Family names are sanitised on entry and on load (`sanitizeFamily`) because they land in a style attribute. `fonts_install` downloads one file over https from the built-in catalogue of 14 OFL/Apache families and writes it to `~/Library/Fonts`; it verifies the sfnt header before writing and never touches a system folder. The picker is `data-action="font-picker"`, choosing is `font-choose` with `data-family`, installing is `font-install` with `data-id`.

## Home: top row, sidebar and library (added 2026-09-16)

Home is `#app[data-screen="home"]`, rendered by `src/workspace/home.ts`. Everything below is a real control on that screen; the editor sections above do not apply until a project is open.

**Top row** (`header.folio-top` → `.folio-top-actions`), left to right:

- **Language**: `details.folio-lang`; the summary shows the active language, the menu holds `button[role=menuitemradio][data-action="set-language"][data-lang="en|ko"]` with `aria-checked`. The editor's `details.top-lang` is the same control.
- **Theme**: `data-action="hub-theme"`, `aria-pressed="true"` when Midnight is on (`.folio-home.folio-night`); the label reads Paper or Midnight.
- **Storage**: `data-action="storage-info"`. Its dot is `i.folio-dot` (plus `.folio-dot-warn` when unsaved) and `small[data-storage-state]` reads Saved / Not saved — the same fact as `#app[data-saved]`. The dialog shows the storage path (`copy-storage-path`), what each file is, and the two downloads `hub-backup` and `hub-recovery`.
- **Import project**: `data-action="import-project"` opens a file picker for a project `.json` (20 MB limit), files it under the active workspace and opens it. Existing projects are kept; a duplicate id is imported as a copy.
- **Agent connection**: `button.folio-toggle[data-action="agent-connect-toggle"]` with `role="switch"` and `aria-checked="true|false"`. On means an agent may edit alongside the person for 12 hours, remembered across relaunches; while on, the button carries `small.folio-toggle-left` with the time left ("for 12 hours · 11 h left") and says the same in its `title`. **This switch is the person's to press.** Pressing it through the agent channel is itself a write, so while it is off an agent's `act` / `click` / `ui` on it comes back **423** — nothing an agent can do opens its own door. Ask instead: `POST /agent/connect` puts a one-line request with the caller's name on the person's screen (`docs/AGENT-CHANNEL.md`).
- **Help**: `details.folio-help > summary` (the `?`), menu `.folio-menu[role=menu]` with **Aphrodite Brand Kit** (`brand-kit`), **Release notes** and **Report an issue**. The editor-only entries (palette, tour, computer-use guide, agent connection, language) are deliberately absent: two of them need a canvas, and the other two already have their own control in this row.

**Sidebar** (`aside.folio-sidebar`): the wordmark is `a.folio-wordmark[data-action="home"]`; the workspace switcher is `details.folio-ws` (see Workspaces above); **Aphrodite Brand Kit** is also `data-action="brand-kit"` in `.folio-side-note`. `data-action="home-sidebar"` collapses and expands it, `.folio-home[data-sidebar="open|collapsed"]` reports which, and when collapsed `button.folio-side-tab` (same action) brings it back.

**Library controls**:

- Filters: `button.folio-filter[data-action="hub-filter"][data-filter="recent|pinned|archived"]`, `aria-pressed` marks the active one and each carries its count. The heading shows the same count zero-padded.
- Search: `#project-search` (`type="search"`). Filtering and search reconcile the grid in place — re-query by `[data-project-id]` rather than assuming fresh nodes.
- View: `button[data-action="hub-view"][data-view="grid|list"]` with `aria-pressed`; the grid is `#project-grid.folio-grid`, and list view adds `.folio-grid-list` and `role="list"`. The choice persists per device.
- A project card is `article.folio-card[data-project-id]` in grid view and `article.folio-row[data-project-id][role=listitem]` in list view. **Open it with the link `a.folio-open[data-action="hub-open"][data-id="<projectId>"]`** (list view: `a.folio-open.folio-row-open`) — the same action in both views. Favourite is `button.folio-pin[data-action="hub-pin"][data-id][aria-pressed]`; `details.folio-more` holds Files (`vault-open`), `hub-rename`, `hub-duplicate`, `hub-archive` and `hub-delete` (which asks, then `hub-delete-confirm`).
- **New project**: `data-action="new-project"` — the hero button, and the button inside the empty state (`.folio-empty`) when a filter or search matches nothing.
- **Recovery row**: a storage or startup error renders `div.folio-warning[role=alert]` with the message and two buttons — `data-action="hub-recovery"` (save the raw storage files) and `data-action="disk-reload"` (re-read the library from disk). It is absent when there is no error, so treat it as a signal, not furniture.
