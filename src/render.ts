...
import {tokenVars} from './design/tokens';
...
function blockContent(b: Block,language:'en'|'ko'='en'): string {
  ...
  switch (b.kind) {
    ...
    case 'features': return `<section class="site-features"><div class="feature-heading"><span class="eyebrow">${l}</span><h2>${t}</h2></div><div class="feature-columns">${b.text.split('\n').map((v, i) => { const [title, text = ''] = v.split('|'); return `<article><span class="feature-symbol">${b.variant==='numbered'?String(i+1).padStart(2,'0'):['✳', '◇', '↗'][i % 3]}</span><h3>${esc(title)}</h3><p>${esc(text)}</p></article>`; }).join('')}</div></section>`;
    ...
    case 'footer': return `<footer class="site-footer" id="contact"><div><span class="site-wordmark">${t}</span><p>${p}</p></div>${b.variant==='columns'?`<div class="footer-columns">${b.text.split('·').slice(0,4).map(v=>`<a href="#top">${esc(v.trim())}</a>`).join('')}</div>`:''}<small>${l}</small></footer>`;
  }
  return '';
}
export function themeVars(p: Project): string {
  ...
  return `--brand:${p.system.accent};--on-brand:${onBrand};--paper:${p.system.background};--ink:${p.system.foreground};--radius:${p.system.radius}px;--heading:${fontStack({family:p.system.headingFamily,category:p.system.font})};--body:${fontStack({family:p.system.bodyFamily,category:'sans'})};${tokenVars(p.system)}`;
}
