...
function assemblyBar(){const page=currentPage(project),last=assemblyRun?.events.at(-1),foreign=assemblyRun&&assemblyRun.projectId!==project.id;return `<div class="assembly-bar-header"><strong>${ui('AGENT ASSEMBLY','에이전트 조립')} · ${assemblyRun?esc(foreign?'다른 프로젝트 · 기록 중지':assemblyRun.status):ui('Ready','준비됨')}</strong><div><button data-action="agent" title="${ui('Assembly brief, run receipts and the computer-use playbook for an external agent','외부 에이전트를 위한 조립 브리프·실행 기록·컴퓨터 유즈 플레이북')}">${ui('Assembly console','조립 콘솔')}</button> <button data-action="pin-frame" ${page.blocks.find(b=>b.id===selected)?.kind==='frame'?'':'disabled'} title="${ui('Keep inserting new components inside the selected Layout frame (select a frame first)','새 컴포넌트를 선택한 레이아웃 프레임 안에 계속 넣습니다 (먼저 프레임을 선택)')}">${ui('Pin selected frame','선택한 프레임에 고정')}</button> <button data-action="insert-root" title="${ui('Insert new components at the top level of the page again','새 컴포넌트를 다시 페이지 최상위에 넣습니다')}">${ui('Insert at page root','페이지 최상위에 삽입')}</button></div></div><div class="assembly-context"><span><strong>${ui('Selected','선택')}</strong> ${esc(nodePath(page.blocks,selected))}</span><span><strong>${ui('Insert into','삽입 위치')}</strong> ${esc(nodePath(page.blocks,insertionTarget()))}</span></div><p class="assembly-receipt ${runStorageWarning?'warning':''}" role="status">${runStorageWarning?esc(runStorageWarning):last?`${ui('Receipt','영수증')} #${last.seq} · ${esc(last.kind)} · ${esc(last.revision)}${assemblyRun!.dropped?` · ${assemblyRun!.dropped} ${ui('events omitted','이벤트 생략')}`:''}`:'Codex가 조립하고, 사람은 방향을 검토합니다. 실행 기록은 Console에서 시작하세요.'}</p>`;}
