...
check('every tool is named, described and annotated honestly', async () => {
  ...
  assert.ok(manifest.length >= 7, `${manifest.length} tools`);
  ...
  assert.deepEqual(reads.sort(), ['aphrodite_get_contract', 'aphrodite_get_tokens', 'aphrodite_list_components', 'aphrodite_list_images']);
